...
export const HotelDetails = ({hotel}:Props) => {
    ...
    console.log(hotel?.missionsCreated?.filter(item => item.status !== "WAITING"))
    return (
        <div className='shadow-normal rounded-[20px] w-full py-[12px] px-5 bg-white'>
            <div className={"flex flex-row gap-4 w-full  py-[12px] "}>
                ...
                <div className="flex flex-col w-full justify-between items-start gap-2">
                    ...
                    <div className="flex flex-row justify-between items-center gap-4 w-full">
                        <p>{format(new Date(hotel?.createdAt!), "dd-MMM-yyyy", {locale:fr})}</p>
            ...
            <div className="flex flex-col gap-5">
                {hotel?.missionsCreated?.filter(item => item.status !== "WAITING")?.map((mission) => (
                        ...
                        <div className="flex justify-between w-full">
                            <div className="flex flex-col text-xs justify-between">
                                <h3 className='text-xs md:text-[20px] text-black font-bold md:font-[400] leading-[29px]'>
                                    {mission.acceptedBy?.firstName} {mission.acceptedBy?.lastName}
                                </h3>
                                <p>{mission.acceptedBy?.address || "Non renseignée"}</p>
                                <p>Shift :
