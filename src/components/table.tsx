...
import { addSpacesToCamelCase } from "@/ui/add-spaces-to-camel-case";
  ...
  return (
    <RcTable
      ...
      columns={(columns ?? []).map((column) => ({
        ...column,
        // @ts-ignore
        render: isLoading ? column.renderSkeleton || column.render : column.render
      })) as any}
      emptyText={
        emptyText || (
          <div className="py-5 text-center lg:py-8">
            <Empty /> <Text className="mt-3">Pas de données</Text>
          </div>
        )
      }
      {...props}
  ...
  return (
    <div>
      <Popover shadow="sm" placement="bottom-end">
        <Popover.Trigger>
          <ActionIcon variant="outline" title={"Masquer des colonnes"}>
