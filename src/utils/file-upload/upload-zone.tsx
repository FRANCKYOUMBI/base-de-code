...
export default function UploadZone({
  ...
  const notUploadedItems = files.filter(
    (file) =>
      !uploadedItems?.some(
        (uploadedFile: FileType) => (uploadedFile as any).file_name === file.name
      )
  );
  ...
  const startUpload = (files: File[]) => {
    ...
    if (files.length > 1) {
      ...
      // get file_name and base64
      const file = files[0];
      const reader = new FileReader();
      reader.onload = async (e) => {
        const base64 = e.target?.result;
          setIsUploading(true);
          const result = await uploadFile(file.name, (base64 ?? "").toString());
          if((result as ServerMessage).statusCode === 500 ) {
            setFiles([]);
            toast.error('Erreur lors de l\'upload du fichier');
          }
          if ((result as {storage: Storage}).storage?.uuid) {
            const uploadedFiles = (result as {storage: Storage}).storage;
            setValue(name, [{...uploadedFiles, file_name: file.name}]);
          }
  ...
  const fileTypes = accept ? (isArray(accept) ? accept : [accept]) : ['image']; /*permittedFileInfo?.config
      ...
      >
        ...
        >
          ...
          <Text className="text-base font-medium">
            Glisser et déposer ou sélectionner un fichier
          </Text>
        </div>
          ...
          {uploadedItems.map((file: any, index: number) => (
            <div key={index} className={cn('relative')}>
              <figure className="group relative h-40 rounded-md bg-gray-50">
                <MediaPreview name={file.name ?? file.file_name} url={file.url} />
                <button
                  type="button"
              ...
              <MediaCaption name={file.name ?? file.file_name} size={file.size} />
            </div>
          ))}
          {notUploadedItems.map((file: any, index: number) => (
            <div key={index} className={cn('relative')}>
              <figure className="group relative h-40 rounded-md bg-gray-50">
                <MediaPreview name={file.name ?? file.file_name} url={file.preview} />
                {isUploading ? (
              ...
              <MediaCaption name={file.path ?? file.file_name} size={file.size} />
            </div>
          ))}
        </div>
      )}
...
function UploadButtons({
  ...
  return (
    <div className="flex w-full flex-wrap items-center justify-center gap-4 px-6 pb-5 @sm:flex-nowrap @xl:w-auto @xl:justify-end @xl:px-0 @xl:pb-0">
      ...
      >
        <PiTrashBold />
        {files.length > 1 ? `Effacer ${files.length} fichiers` : 'Effacer'}
      </Button>
      <Button
        className="w-full gap-2 @xl:w-auto"
        isLoading={isLoading}
        onClick={onUpload}
      >
        <PiUploadSimpleBold />  {files.length > 1 ? `Upload {files.length} files` : 'Uploader' }
      </Button>
    </div>
  );
}
...
function MediaPreview({ name, url }: { name: string; url: string }) {
  ...
  ) : (endsWith(name, '.svg') || endsWith(name, '.webp') || endsWith(name, '.png') || endsWith(name, '.jpg') || endsWith(name, '.jpeg')) ? (
    <Image
      fill
      loading={'eager'}
      src={url}
