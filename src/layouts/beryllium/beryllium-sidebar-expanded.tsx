...
function LinkMenuItem({ item, lang }: { item: ItemType; lang?: string }) {
  ...
  const isActive = item.isCurrent(pathname);
...
function CollapsibleMenuItem({
  ...
  const pathnameExistInDropdowns: any = item?.subMenuItems?.filter(
    (dropdownItem) => dropdownItem.isCurrent(pathname)
  );
  const isDropdownOpen = Boolean(pathnameExistInDropdowns?.length);
  const isActive = item.subMenuItems?.some(
    (subMenuItem) => subMenuItem.isCurrent(pathname)
  );
  ...
  return (
    ...
    >
      {item?.subMenuItems?.map((subMenuItem, index) => {
        const isChildActive = subMenuItem.isCurrent(pathname);
        return (
          <Link
          ...
          >
            <div className="flex items-center truncate">
              ...
              />
              <span className="truncate normal-case">{t(subMenuItem?.name)}</span>
            </div>
