...
import { Role, rolePermissions } from '@/services/types/role-permissions';
import { filterMenuItemsByPermissions } from '@/ui/filter-menu-items';
...
function MenuItem({ menu, lang }: { menu: MenuItemsType; lang?: string }) {
  ...
  return (
    ...
    >
      ...
      <span className="text-white text-center">{t(menu.name)}</span>
    </li>
  );
}

// Composant MenuItems
function MenuItems({ lang, userRole }: { lang?: string; userRole: Role }) {
  const [menuItems] = useAtom(berylliumMenuItemAtom);
  const userPermissions = rolePermissions[userRole]; // Récupérer les permissions de l'utilisateur
  const filteredMenuItems = filterMenuItemsByPermissions(berylliumMenuItems, userPermissions); // Filtrer les éléments du menu
...
// Composant principal BerylliumLeftSidebarFixed
export default function BerylliumLeftSidebarFixed({
  ...
  const userPermissions = rolePermissions[userRole]; // Récupérer les permissions de l'utilisateur
  const filteredMenuItems = filterMenuItemsByPermissions(berylliumMenuItems, userPermissions); 
  ...
  useEffect(() => {
    ...
    setMenuItems(filteredMenuItems[activeMenuIndex]);
  ...
  return (
    <aside className="fixed start-0 top-0 z-50 hidden h-screen w-[88px] flex-col items-center gap-10 bg-gray-900 py-3.5 xl:flex dark:bg-gray-0">
      ...
      <MenuItems lang={lang} userRole={userRole} />
    </aside>
  );
}